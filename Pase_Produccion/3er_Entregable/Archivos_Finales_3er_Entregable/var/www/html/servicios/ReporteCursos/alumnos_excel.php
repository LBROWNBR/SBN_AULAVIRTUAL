...
$objPHPExcel->getActiveSheet()->setCellValue('Q1', 'DEPARTAMENTO');
$objPHPExcel->getActiveSheet()->setCellValue('R1', 'PROVINCIA');
$objPHPExcel->getActiveSheet()->setCellValue('S1', 'DISTRITO');
...
$objPHPExcel->getActiveSheet()->setCellValue('AB1', '¿TIENE CERTIFICADO?');
$objPHPExcel->getActiveSheet()->setCellValue('AC1', 'CODIGO CERTIFICADO');

$lastColumnCAB = 'AC';
foreach ($mObjActividades as $indice01 => $regAct):        
    $lastColumnCAB++;
    $Nom_columna = $mObjActividades->$indice01['NOMACTIVIDAD'];     
    $objPHPExcel->getActiveSheet()->setCellValue($lastColumnCAB . '1', $Nom_columna);
endforeach;

$objPHPExcel->getActiveSheet()->getStyle('A1:'.$lastColumnCAB.'1')->applyFromArray($estiloTituloColumnas);
...
foreach ($datos as $data) {
    ...
    $ArrayNotasDet = $data[31]; //mdl_NOTAS
...
$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A2:".$lastColumnDET . $fila);
