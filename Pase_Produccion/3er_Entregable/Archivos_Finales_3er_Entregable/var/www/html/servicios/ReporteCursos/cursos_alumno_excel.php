...
$x01_Actividades = isset($_SESSION['reporte_actividades']) ? $_SESSION['reporte_actividades'] : die();

$mObjActividades = (object) $x01_Actividades;

$objPHPExcel->getActiveSheet()->setCellValue('A2', 'N°');
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'NOMBRE CORTO CURSO');
...
$objPHPExcel->getActiveSheet()->setCellValue('AE2', 'CODIGO POI');
$objPHPExcel->getActiveSheet()->setCellValue('AF2', '¿TIENE CERTIFICADO?');
$objPHPExcel->getActiveSheet()->setCellValue('AG2', 'CODIGO CERTIFICADO');

$lastColumnCAB = 'AG';
foreach ($mObjActividades as $indice01 => $regAct):        
    $lastColumnCAB++;
    $Nom_columna = $mObjActividades->$indice01['NOMACTIVIDAD'];     
    $objPHPExcel->getActiveSheet()->setCellValue($lastColumnCAB . '2', $Nom_columna);
endforeach;

$objPHPExcel->getActiveSheet()->getStyle('A2:'.$lastColumnCAB.'2')->applyFromArray($estiloTituloColumnas);
...
foreach ($datos['datos'] as $data) {
    ...
    $ArrayNotasDet = $data[33]; //mdl_NOTAS
    ...
    foreach ($mObjActividades as $xIndice => $regActividad):
        ...
        if($ArrayNotasDet){
            if(isset($ArrayNotasDet[$xIndice]['NOTA'])){
                $objPHPExcel->getActiveSheet()->setCellValue($lastColumnDET . $fila, $ArrayNotasDet[$xIndice]['NOTA']);
            }else{
                $objPHPExcel->getActiveSheet()->setCellValue($lastColumnDET . $fila, '0');
            }                                    
        }else{
            $objPHPExcel->getActiveSheet()->setCellValue($lastColumnDET . $fila, '0');
        }
...
$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A3:".$lastColumnDET . $fila);
$objPHPExcel->getActiveSheet()->getStyle("A2:B{$fila}")->applyFromArray($styleCenter);

$objPHPExcel->getActiveSheet()->getStyle("A2:A".$lastColumnDET."{$fila}")->getAlignment()->setWrapText(true);

foreach (range('A', $lastColumnDET) as $columnID) {
...
$objPHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth("90");
...
foreach ($mObjActividades as $indice01 => $regAct):        
    $lastColumnCAB++;
    $Nom_columna = $mObjActividades->$indice01['NOMACTIVIDAD'];
    $TotalCaracter = strlen($Nom_columna);
    $objPHPExcel->getActiveSheet()->getColumnDimension($lastColumnCAB)->setWidth($TotalCaracter);
endforeach;
...
$objPHPExcel->getActiveSheet()->getStyle('A1:'.$lastColumnDET.'1')->applyFromArray($styleTitulo);
$objPHPExcel->getActiveSheet()->mergeCells('A1:'.$lastColumnDET.'1');
