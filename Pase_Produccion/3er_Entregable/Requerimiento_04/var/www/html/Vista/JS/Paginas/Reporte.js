...
//============================================
//REPORTE 08 - Datos Abiertos
//============================================

var reporte_DatosAbiertos = () => {
    if (!validOnlyRequired('cboAnio_R8')) {
        return;
    }
    if (!validOnlyRequired('cboCursos_R8')) {
        return;
    }
    $.post("../app/controller/reporte_moodle.php", {
        op: 'reporte_datos_abiertos',
        id_curso: $("#cboCursos_R8").val(),
    }, function(data) {
        if (data == true) {
            window.location.href = '../servicios/ReporteCursos/datos_abiertos_excel.php';
        } else {
            SwalError('Oops...', 'No existen practicas realizadas en este curso!');
        }
    });
